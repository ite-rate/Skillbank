...
def test_teleagent_zh_desc_mirrored_to_cn(agents_cfg, tmp_path):
...
def test_teleagent_manual_level_enabled_at_false(agents_cfg, tmp_path):
...
def test_teleagent_canonical_meta_not_polluting(agents_cfg, tmp_path):
...
def test_qwenworkcn_zh_desc_direct_pass(agents_cfg, tmp_path):
...
def test_qwenworkcn_no_obsolete_fields(agents_cfg, tmp_path):
...
def test_codex_description_truncated_at_1024(agents_cfg, tmp_path):
...
def test_codex_description_at_boundary_not_truncated(agents_cfg, tmp_path):
...
def test_codex_unicode_chars_counted_not_bytes(agents_cfg, tmp_path):
    ...
    result2 = em.deploy(ir2, tmp_path / "x2", cfg, tmp_path / "c2" / "demo")
    fm2, _ = _split_frontmatter_body(result2.deployed_path.read_bytes())
    assert len(fm2["description"]) <= CODEX_DESC_MAX


def test_codex_manual_level_disable_model_invocation_true(agents_cfg, tmp_path):
...
def test_hermes_deploys_into_category_subdir(agents_cfg, tmp_path):
    """Hermes 默认走 imported/<name>/ 子目录(决策:不污染 creative/)。"""
    ir = _make_ir()
    cfg = agents_cfg.get("Hermes")
    em = HermesEmitter()
    result = em.deploy(ir, tmp_path / "hermes-skills", cfg, tmp_path / "c" / "demo")
    assert "imported/demo/SKILL.md" in str(result.deployed_path)
    assert result.method == "cp"


def test_hermes_body_zero_loss_in_category(agents_cfg, tmp_path):
...
def test_hermes_manual_level_metadata_hermes_namespace(agents_cfg, tmp_path):
...
def test_hermes_description_truncated(agents_cfg, tmp_path):
...
def test_hermes_oversize_file_skipped(agents_cfg, tmp_path):
...
def test_hermes_oversize_one_agent_while_others_sync(tmp_path, agents_cfg):
    ...
    result_codex = codex_em.deploy(ir, tmp_path / "codex", codex_cfg, tmp_path / "c3" / "demo")
    ...
    result_hermes = hermes_em.deploy(ir, tmp_path / "hermes", hermes_cfg, tmp_path / "c3" / "demo")
