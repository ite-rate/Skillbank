...
class xml_editor_impl:
    ...
    def get_node_impl(
        ...
        """
        Get a DOM element by tag and filters.

        Finds elements matching the given filters. When multiple elements match,
        returns the one at the specified index instead of raising an error.
        ...
        Args:
            ...
            index: Which match to return when multiple elements match (default: 0, first match)

        Returns:
            defusedxml.minidom.Element: The matching DOM element

        Raises:
            ValueError: If node not found or index out of range
        ...
        if not matches and contains is not None and line_number is not None:
            matches = self._fallback_contains_search(tag, line_number, contains)
        ...
        if len(matches) > 1:
            if index >= len(matches):
                raise ValueError(
                    f"Index {index} out of range: only {len(matches)} nodes found for <{tag}>. "
                    f"Valid indexes: 0-{len(matches) - 1}"
                )
    ...
    def _fallback_contains_search(self, tag, line_number, contains):
        """Fallback text search using toxml() when _get_element_text fails.

        Handles edge cases where XML structure causes _get_element_text() to miss
        text that is visible when manually inspecting the DOM.
        """
        import re
        ...
        for elem in candidates:
            raw = html.unescape(elem.toxml())
            text = re.sub(r"<[^>]+>", "", raw)
            if normalized_contains in text:
                matches.append(elem)
