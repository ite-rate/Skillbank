...
from ooxml.scripts.pack_workflow import pack_document
from ooxml.scripts.validation.docx_workflow import docx_schema_validator_impl
from ooxml.scripts.validation.redlining_workflow import redlining_validator_impl

from .utilities_workflow import xml_editor_impl
...
class document_impl:
    ...
    def __init__(
        ...
        Args:
            ...
            author: Default author name for comments (default: "TeleAgent")
        ...
        pack_document(self.original_path, self.original_docx, validate_impl=False)
    ...
    def __getitem__(self, xml_path: str) -> docx_xml_editor_impl:
        """
    ...
    def add_comment_impl(self, start, end, text: str) -> int:
        ...
        self._document.insert_before_impl(start, self._comment_range_start_xml(comment_id))

        # If end node is a paragraph, append comment markup inside it
        # Otherwise insert after it (for run-level anchors)
        if end.tagName == "w:p":
            self._document.append_to_impl(end, self._comment_range_end_xml(comment_id))
        else:
            self._document.insert_after_impl(end, self._comment_range_end_xml(comment_id))
    ...
    def reply_to_comment_impl(
        ...
        # Add comment ranges to document.xml immediately
        parent_start_elem = self._document.get_node_impl(
            tag="w:commentRangeStart", attrs={"w:id": str(parent_comment_id)}
        )
        parent_ref_elem = self._document.get_node_impl(
            tag="w:commentReference", attrs={"w:id": str(parent_comment_id)}
        )

        self._document.insert_after_impl(
    ...
    def validate_impl(self) -> None:
        ...
        schema_validator = docx_schema_validator_impl(
            self.unpacked_path, self.original_docx, verbose=False
        )
        redlining_validator = redlining_validator_impl(
            self.unpacked_path, self.original_docx, verbose=False
        )

        # Run validations
        if not schema_validator.validate_impl():
            raise ValueError("Schema validation failed")
        if not redlining_validator.validate_impl():
            raise ValueError("Redlining validation failed")

    def save_impl(self, destination=None, validate_impl=True) -> None:
        """
    ...
    def _add_content_type_for_people(self, path):
        ...
        editor.append_to_impl(root, override_xml)
    ...
    def _add_relationship_for_people(self, path):
        ...
        next_rid = editor.get_next_rid_impl()
        ...
        editor.append_to_impl(root, rel_xml)
    ...
    def _update_settings(self, path, track_revisions=False):
        ...
        root = editor.get_node_impl(tag="w:settings")
        ...
        if track_revisions:
            ...
            if not track_revisions_exists:
                ...
                for tag in [f"{prefix}:documentProtection", f"{prefix}:defaultTabStop"]:
                    ...
                    if elements:
                        editor.insert_before_impl(elements[0], track_rev_xml)
                        inserted = True
                        break
                if not inserted:
                    # Insert as first child of settings
                    if root.firstChild:
                        editor.insert_before_impl(root.firstChild, track_rev_xml)
                    else:
                        editor.append_to_impl(root, track_rev_xml)
  ...
  <{prefix}:rsid {prefix}:val="{self.rsid}"/>
            ...
            if compat_elements:
                editor.insert_after_impl(compat_elements[0], rsids_xml)
                inserted = True
            ...
            if not inserted:
                ...
                if clr_elements:
                    editor.insert_before_impl(clr_elements[0], rsids_xml)
                    inserted = True

            if not inserted:
                editor.append_to_impl(root, rsids_xml)
        else:
            ...
            if not rsid_exists:
                ...
                editor.append_to_impl(rsids_elem, rsid_xml)
    ...
    def _add_to_comments_xml(
        ...
        editor = self["word/comments.xml"]
        root = editor.get_node_impl(tag="w:comments")
  ...
  <w:p w14:paraId="{para_id}" w14:textId="77777777">
    ...
    <w:r><w:rPr><w:color w:val="000000"/><w:sz w:val="20"/><w:szCs w:val="20"/></w:rPr><w:t>{escaped_text}</w:t></w:r>
        ...
        editor.append_to_impl(root, comment_xml)
    ...
    def _add_to_comments_extended_xml(self, para_id, parent_para_id):
        ...
        root = editor.get_node_impl(tag="w15:commentsEx")
        ...
        editor.append_to_impl(root, xml)
    ...
    def _add_to_comments_ids_xml(self, para_id, durable_id):
        ...
        root = editor.get_node_impl(tag="w16cid:commentsIds")
    ...
    def _add_to_comments_extensible_xml(self, durable_id):
        ...
        root = editor.get_node_impl(tag="w16cex:commentsExtensible")
...
<w:r>
  ...
  <w:commentReference w:id="{comment_id}"/>
    ...
    def _add_author_to_people(self, author):
        ...
        root = editor.get_node_impl(tag="w15:people")
  ...
  <w15:presenceInfo w15:providerId="None" w15:userId="{escaped_author}"/>
        ...
        editor.append_to_impl(root, person_xml)
    ...
    def _ensure_comment_relationships(self):
        ...
        root_tag = root.tagName  # type: ignore
        prefix = root_tag.split(":")[0] + ":" if ":" in root_tag else ""
        next_rid_num = int(editor.get_next_rid_impl()[3:])
...
class docx_xml_editor_impl(xml_editor_impl):
    ...
    def __init__(
        self, xml_path, rsid: str, author: str = "TeleAgent", initials: str = "C"
    ):
        ...
        Args:
            ...
            author: Author name for tracked changes and comments (default: "TeleAgent")
    ...
    def replace_node_impl(self, elem, new_content):
        """Replace node with automatic attribute injection."""
        nodes = super().replace_node_impl(elem, new_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def insert_after_impl(self, elem, xml_content):
        """Insert after with automatic attribute injection."""
        nodes = super().insert_after_impl(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def insert_before_impl(self, elem, xml_content):
        """Insert before with automatic attribute injection."""
        nodes = super().insert_before_impl(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def append_to_impl(self, elem, xml_content):
        """Append to with automatic attribute injection."""
        nodes = super().append_to_impl(elem, xml_content)
        self._inject_attributes_to_nodes(nodes)
        return nodes

    def revert_insertion_impl(self, elem):
    ...
    def revert_deletion_impl(self, elem):
        ...
        for del_elem in del_elements:
            ...
            nodes = self.insert_after_impl(del_elem, ins_elem.toxml())
    ...
    @staticmethod
    def suggest_paragraph_impl(xml_content: str) -> str:
    ...
    def suggest_deletion_impl(self, elem):
