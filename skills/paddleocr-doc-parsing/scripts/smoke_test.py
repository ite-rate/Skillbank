...
from config_store import get_config_path, get_keyring_backend_name
...
2. Register and sign in. For domestic users, mobile phone registration is the easiest path.
3. Open your model's API page
4. Open the Example Code section
5. Copy the API URL value
6. Copy the Access Token value

Notes for new users:
  - New accounts usually have a daily free quota, which is enough for light personal office use.
  - If you want to avoid entering credentials every time, save them once with:
    python scripts/configure_credentials.py --api-url "https://paddleocr.aistudio-app.com/api/v2/ocr/jobs" --access-token "your_token_here"

Set environment variables:
  export PADDLEOCR_DOC_PARSING_API_URL=https://paddleocr.aistudio-app.com/api/v2/ocr/jobs
...
def main():
    ...
    try:
        ...
        kr_ver = getattr(keyring, "__version__", "installed")
        print(f"  + keyring: {kr_ver}")
    except ImportError:
        print("  X keyring not installed")
        print("\nPlease install dependencies:")
    ...
    try:
        ...
        print(f"  + Persisted config file: {get_config_path()}")
