...
def print_error_summary(result):
    """Print a concise error summary to stderr for interactive use."""
...
Or persist credentials once for future runs:
  python scripts/configure_credentials.py --api-url "https://paddleocr.aistudio-app.com/api/v2/ocr/jobs" --access-token "token"
        """,
    )
    ...
    parser.add_argument(
        ...
        type=int,
        choices=[0, 1],
        help="Optional file type hint (0=PDF, 1=Image). The API can auto-detect.",
    )
    parser.add_argument(
        "--model",
        help="OCR model name (e.g. PaddleOCR-VL-1.6). Defaults to PaddleOCR-VL-1.6.",
    )
