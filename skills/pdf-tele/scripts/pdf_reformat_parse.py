...
def _md_inline(text: str) -> str:
    ...
    # Protect inline code before emphasis conversion so filenames like
    # `page_1.png` are not mistaken for underscore italics.
    code_spans: list[str] = []

    def code_repl(match):
        code_spans.append(
            f'<font name="Courier">{html.escape(match.group(1))}</font>'
        )
    ...
    # Escape raw XML-ish characters before adding ReportLab markup.
    text = html.escape(text)

    # Bold: **text** or __text__
    text = re.sub(r'\*\*(.+?)\*\*', r'<b>\1</b>', text)
    text = re.sub(r'__(.+?)__',     r'<b>\1</b>', text)
    # Italic: *text*. Underscore italics are intentionally not supported
    # because they collide with common filenames and identifiers.
...
def main():
    ...
    parser.add_argument("--out", "--output", dest="out", default="content.json", help="Output content.json path")
