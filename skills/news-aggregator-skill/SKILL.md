...
description_zh: 自动追踪科技、创投、金融领域的热点资讯，整合Hacker News、36氪、华尔街见闻等8大专业平台，智能提炼要点并生成每日/每周简报，一键获取来源、摘要、深度解读。
name_zh: AI新闻周报
---
# News Aggregator Skill

Fetch real-time hot news from multiple sources.
## Relative Time Processing (Mandatory)

When encountering relative time expressions such as "February of this year," "last month," or "the last 7 days," you must first parse them using the runtime date. Relying on memory to determine the year is strictly prohibited.
The time zone is fixed to Asia/Shanghai.
...
Requirements:
- "February of this year" = February of the current year.
- The parsed year and month must serve as the single source of truth, used uniformly across searches, titles, body text, and the final response.
- Before generating the output, you must verify time consistency throughout the entire text; if any inconsistencies exist, correct them based on the runtime results.
